        ...
        {           
            ...
            if (ModelState.IsValid)
            {
                ...
                if (usuario.IdUsuario == 0)
                {
                    ...
                    if (result.Correct)
                    {
                        ViewBag.MessageCorrect = "El usuario se agrego correctamente";
                        return PartialView("_Mensajes");
                    }
                    else
                    {
                        ViewBag.MessageFalse = "El usuario no se pudo agregar";
                        return PartialView("_Mensajes");
                    }
                ...
                else
                {
                    ...
                    if (usuario.Direccion.IdDireccion != 0)
                    {
                        ...
                        {
                            ViewBag.MessageFalse = "El usuario no se pudo actualizar";
                            return PartialView("_Mensajes");
                        }
                    }
                    else
                    {
                        ML.Result result = BL.Usuario.UusarioSinDireccionUpdate(usuario);
                        if (result.Correct)
                        {
                            ViewBag.MessageCorrect = "El usuario se actualizo correctamente";
                            return PartialView("_Mensajes");
